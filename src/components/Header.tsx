...
export function useShrunk() {
  ...
  useEffect(() => {
    const onScroll = () => setShrunk(window.scrollY > SHRINK_THRESHOLD);
...
export default function Header({ shrunk }: { shrunk: boolean }) {
  ...
  return (
    <header
      className={`text-center px-4 relative transition-all duration-300 ease-out
        ${shrunk
          ? "pt-2 pb-1"
          : "pt-8 pb-4"
        }`}
    >
      ...
      {/* Subtle neon glow behind title — hidden when shrunk */}
      <div
        className={`absolute top-4 left-1/2 -translate-x-1/2 w-64 h-16 rounded-full blur-3xl pointer-events-none
          transition-opacity duration-300 ${shrunk ? "opacity-0" : "opacity-20"}`}
      ...
      >
        <img
          src="/icon-192.png"
          alt=""
          className={`rounded-lg opacity-80 transition-all duration-300 ${shrunk ? "w-6 h-6" : "w-12 h-12"}`}
          aria-hidden="true"
        />
        <h1
          className={`font-bold neon-text tracking-wider transition-all duration-300
      ...
      {/* Alias, subtitle, decorative line — hidden when shrunk */}
      <div className={`transition-all duration-300 overflow-hidden ${shrunk ? "max-h-0 opacity-0" : "max-h-24 opacity-100"}`}>
        {t.header.alias && (
